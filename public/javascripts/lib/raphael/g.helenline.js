...
*/
        var pathX = paper.path( ['M', 0, 0, 'L', 0, height] ).attr({stroke: '#A6A6A6', 'stroke-width': 1.5});
        axis.push(pathX);
        var pathY = paper.path( ['M', width, 0, 'L', width, height] ).attr({stroke: '#A6A6A6', 'stroke-width': 1.5});
        axis.push(pathY);
        
        for(var i = 0; i < 10; ++i ) {
	        var stepLine = paper.path( ['M', (width/10)*i, height-5, 'L', (width/10)*i, height] ).attr({stroke: '#A6A6A6', 'stroke-width': 1.5});
	        axis.push(stepLine);
	    };
        ...
        for (i = 0, ii = valuesy.length; i < ii; i++) {
            if (!opts.nostroke) {
                lines.push(line = paper.path().attr({
                    stroke: colors[i],
                    "stroke-width": opts.width[i] || 2,
                    "stroke-linejoin": "round",
                    "stroke-linecap": "round",
                    "stroke-dasharray": opts.dash[i] || ""
                }));
            }
