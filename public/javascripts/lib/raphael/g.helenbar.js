...
(function () {
    ...
    function VBarchart(paper, x, y, width, height, values, opts) {
        ...
        chart.p2c = function(p) {
        	return { x: p.x * (barwidth + barhgutter), y: height - p.y * Y - barvgutter };
        };
    ...
    function HBarchart(paper, x, y, width, height, values, opts) {
        ...
        var chartinst = this,
            ...
            colors = opts.colors || chartinst.colors,
            len = values.length,
            barHeightFraction = opts.barHeightFraction || 1.0;
        ...
        var axis = paper.set();
        for(var i = 0; i < 10; ++i ) {
 	       var stepLine = paper.path( ['M', (width/10)*i, 20, 'L', (width/10)*i, height-20] ).attr({stroke: 'rgba(0,0,0,0.5)', 'stroke-width': 1.5});
 	       i && paper.text((width/10)*i, height-10, (total/10)*i);
 	       axis.push(stepLine);
 	    };
